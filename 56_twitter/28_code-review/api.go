...
func SubmitTweet(res http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	ctx := appengine.NewContext(req)
	s := getSession(req)

	var user User
	err := json.Unmarshal(s.Value, &user)
	...
	tweet := Tweet {
		...
		Text: req.FormValue("tweetText"),
	}

	key := datastore.NewKey(ctx, "Tweet", tweet.UserName, 0, nil)
...
func loginProcess(res http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	if err != nil || req.FormValue("password") != user.Password {
		// failure logging in
		var sd SessionData // was sessionData
		sd.LoginFail = true
